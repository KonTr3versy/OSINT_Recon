...
async def run(domain: str, company: str | None, http: HttpClient, max_results: int = 10) -> dict:
    users: list[dict] = []
    warnings: list[str] = []
    per_source_counts = {"github_search": 0, "keybase_autocomplete": 0}

    terms = _query_terms(domain, company)
    for term in terms:
        gh_users, gh_err = await _github_users(term, domain, http, max_results=max_results)
        if gh_err:
            warnings.append(gh_err)
        else:
            users.extend(gh_users)
            per_source_counts["github_search"] += len(gh_users)

        kb_users, kb_err = await _keybase_users(term, domain, http, max_results=max_results)
        if kb_err:
            warnings.append(kb_err)
        else:
            users.extend(kb_users)
            per_source_counts["keybase_autocomplete"] += len(kb_users)

    # dedupe by source+handle while preserving order
    deduped: list[dict] = []
    seen = set()
    for user in users:
    ...
    return {
        ...
        "warnings": warnings,
        "attribution": {
            "sources": ["github_search", "keybase_autocomplete"],
